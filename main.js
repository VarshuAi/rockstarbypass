...
// ── Custom paths ────────────────────────────────────────────────────
const customPathsFile = path.join(os.homedir(), 'AppData', 'Local', 'vortex-launcher', 'custom_paths.json');
...
const keyFilePath = path.join(os.homedir(), 'AppData', 'Local', 'vortex-launcher', 'access_key.txt');
...
ipcMain.handle('get-unlocked-achievements', async () => {
  const achievementDir = path.join(os.homedir(), 'AppData', 'Local', 'vortex-launcher', 'achievement_pipe');
...
ipcMain.handle('start-bypass', async (event, { gameName, gamePath }) => {
  ...
  const backupDir = path.join(os.homedir(), 'AppData', 'Local', 'vortex-launcher', 'OriginalFiles', sanitizeForPath(gameName));
  if (!fs.existsSync(backupDir)) fs.mkdirSync(backupDir, { recursive: true });

  try {
    if (game.zip) {
      const tempDir = path.join(os.tmpdir(), `vortex-bypass-${Date.now()}`);
...
ipcMain.handle('verify-bypass', async (event, { gameName, gamePath }) => {
  ...
  try {
    if (game.zip) {
      const tempDir = path.join(os.tmpdir(), `vortex-verify-${Date.now()}`);
      const zipPath = path.join(tempDir, 'fix.zip');
